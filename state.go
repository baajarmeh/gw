...
func gwState(serverName string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		defer func() {
			...
			if ok {
				for _, handler := range handlers {
					handler(requestId, status, string(httpRequest), headers, string(stacks), errOriBody, c.Errors)
				}
			}
			// After handlers.
		...
		if ok {
			c.Set(gwSidKey, sid)
			user, err := s.SessionStateManager.Query(sid)
...
func getSid(s *HostServer, c *gin.Context) (string, bool) {
	...
	sidKey := s.conf.Security.Auth.TrustSidKey
	if sidKey != "" {
